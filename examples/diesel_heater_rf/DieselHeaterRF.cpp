...
uint32_t DieselHeaterRF::parseAddress(char *buf) {
  uint32_t address = 0;
  //Serial.println("Starting parseAddress");
  address |= (buf[2] << 24);
  address |= (buf[3] << 16);
  address |= (buf[4] << 8);
  address |= buf[5];
...
bool DieselHeaterRF::receivePacket(char *bytes, uint16_t timeout) {
  ...
  rxFlush();
  rxEnable();
  //Serial.println("Starting Recieve packet ");
  ...
  uint16_t crc = crc16_2(bytes, 21);
  if (crc == (bytes[21] << 8) + bytes[22]) {
    //Serial.println("CRC True");
    return true;
  }
...
void DieselHeaterRF::initRadio() {
  ...
  /*Original frequency set to 433.999
   * Tested heater uses 433.946
   */
  /*
  ...
  */
  //New values for 433.946
  writeReg(0x0D, 0x10); // FREQ2
  writeReg(0x0E, 0xB0); // FREQ1
  writeReg(0x0F, 0xB2); // FREQ0
